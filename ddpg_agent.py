...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed, params):
        ...
        #self.critic_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.critic_optimizer, mode='max', patience=2)
        ...
        if Agent.memory is None:
            ...
            Agent.memory = ReplayBuffer(action_size, self.buffer_size, self.batch_size, random_seed)

    def step(self, t_step, state, action, reward, next_state, done, completed, mean_score):
        ...
        if len(Agent.memory) > self.batch_size and t_step % self.time_steps == 0:
            #self.critic_scheduler.step(mean_score)
            for i in range(self.learn_updates):
                experiences = Agent.memory.sample(completed)
    ...
    def learn(self, experiences, gamma):
        ...
        self.learn_count += len(experiences)
